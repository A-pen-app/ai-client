...
(重要總則)： 以下所有 4 個標籤類別（工作類型、需求科別、需求職級、職缺地點）都必須以「陣列 (Array)」的格式回傳，因為一個職缺可能同時擁有多個標籤。即使只找到一個值，也請用陣列回傳。
1. 辨識工作類型：
根據內文掃描 [全職] 或 [兼職] 相關關鍵字。一個職缺可能兩者皆可（例如：文中提及「全職或兼職均可」），請回傳所有找到的適用類型。回傳值請使用數字：全職 = 0，兼職 = 1。 (範例：若提及「全職/兼職」，應回傳 [0, 1])
2. 辨識需求科別 (站內資料及規則強化)：
A. 站內科別清單 (優先比對項目)： 請主要依據以下清單來比對並萃取文中所有提及的科別： "內科", "胸腔科", "心臟科", "腎臟科", "肝膽腸胃科", "血液腫瘤科", "內分泌新陳代謝科", "免疫風濕科", "感染科", "神經內科", "家庭醫學科", "復健科", "精神科", "外科", "一般外科", "心臟外科", "胸腔外科", "整形外科", "神經外科", "大腸直腸外科", "小兒外科", "移植外科", "創傷外科", "婦產科", "眼科", "耳鼻喉科", "骨科", "泌尿科", "皮膚科", "麻醉科", "急診", "小兒科", "不分科", "牙科", "一般牙科", "牙體復形科", "口腔外科", "齒顎矯正科", "口腔診斷科", "牙髓病科", "牙周病科", "贋復假牙科", "家庭牙科", "補綴科", "特殊需求牙科", "兒童牙科", "中醫師", "職業醫學科", "病理科", "核子醫學科", "放射腫瘤科", "影像醫學科", "重症科", "老年醫學科"
B. 額外辨識與修正規則：
...
B. 如果「沒有」明確提及 (不限情況)： 如果內文（例如：只寫「誠徵醫師」或未提及職級）完全沒有包含步驟 1 的任何關鍵字，表示此職缺不限資格，請「預設」回傳所有三個職級：["PGY", "Resident", "VS"]。
C. 廣泛範圍補足 (優化規則)： 若步驟 2A 的結果中，同時包含 PGY 類標籤 和 主治醫師類標籤，則必須補足「Resident」，確保回傳 ["PGY", "Resident", "VS"]。
回傳值請使用以下代碼：PGY = "PGY"，住院醫師 = "Resident"，主治醫師 = "VS"。
4. 辨識職缺地點 (新規則強化)：
...
(重要總則)： 以下所有 2 個標籤類別（工作類型、職缺地點）都必須以「陣列 (Array)」的格式回傳，因為一個職缺可能同時擁有多個標籤。即使只找到一個值，也請用陣列回傳。
1. 辨識工作類型：
根據內文掃描 [全職] 或 [兼職] 相關關鍵字。一個職缺可能兩者皆可（例如：文中提及「全職或兼職均可」），請回傳所有找到的適用類型。回傳值請使用數字：全職 = 0，兼職 = 1。 (範例：若提及「全職/兼職」，應回傳 [0, 1])
2. 辨識職缺地點 (新規則強化)：
萃取文中的所有工作地點。不要只抓「主要」或「第一個」地點。
